...
- **Layer 3: resolution chain** (`src/embeddings/metadata-resolver.ts`) — pure function with all deps injected. Order: hot cache → bundled seed → HF live → embedder probe → safe defaults. `OBSIDIAN_BRAIN_REFETCH_METADATA=1` forces synchronous refetch. Sync variant (`resolveModelMetadataSync`) for the bootstrap prefix-strategy hash.
- **Layer 4: cache persistence** (`src/embeddings/metadata-cache.ts`) — schema v7 columns on `embedder_capability` (`dim`, `query_prefix`, `document_prefix`, `prefix_source`, `base_model`, `size_bytes`, `fetched_at`). 90-day TTL with stale-while-revalidate (return cached + fire async refetch).
...
- **Smoke-tested `npm run build:seed` end-to-end** (1m43s, 309 models from a real `embeddings-benchmark/results` clone + HF API). The pipeline works. Surfaced two correctness gaps that mean **release-time auto-regen is DEFERRED to v1.7.6**: (a) MTEB indexes by upstream HF id only, so `Xenova/bge-small-en-v1.5` mirrors that our presets reference are absent from a pure auto-seed; (b) ~79% of MTEB models pre-date the `config_sentence_transformers.json` prompts convention — their prefixes live in README prose only, so pure HF resolution returns `prefixSource='none'` and silently strips the prefix. The `release.yml` build-seed step is commented out until v1.7.6 adds anchor-overlay logic + Xenova-mirror discovery; v1.7.5 ships the hand-curated 6-entry anchor at `data/seed-models.json`. BYOM users with models that DO ship structured prompts (mdbr-leaf-ir, multilingual-e5, snowflake-arctic-v2, etc.) get correct prefixes via the runtime metadata-resolver chain on first use.
