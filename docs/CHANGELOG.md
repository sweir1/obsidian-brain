...
- **Smoke-tested `npm run build:seed` end-to-end** (1m43s, 309 models from a real `embeddings-benchmark/results` clone + HF API). The pipeline works. Xenova-mirror discovery added to `scripts/build-seed.mjs` (auto-derives `Xenova/<basename>` from each upstream MTEB id and probes it) so the seed includes the ids our presets reference — closes the Xenova-absent gap. Tier 3 README fingerprinting closes the BGE-prefix-in-prose gap. The remaining v1.7.6 work is HF rate-limit handling: a full ~700-candidate run with mirrors burns ~5,500 API calls in 90s vs free-tier 1,000/5min. Plan: read `HF_TOKEN` GitHub Actions secret in `build-seed.mjs`, parse `RateLimit` response headers for 429-aware backoff, then re-enable the auto-regen step in `release.yml`. Until that lands, the committed hand-curated 6-entry anchor at `data/seed-models.json` ships as-is; runtime resolver picks up correct prefixes via Tier 3 for BYOM models the anchor doesn't cover.
