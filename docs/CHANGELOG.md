...
- **`data/seed-models.json` is regenerated at every release from the MTEB Python registry — zero HF API calls** (`scripts/build-seed.py`). The previous `scripts/build-seed.mjs` (Node) cloned `embeddings-benchmark/results`, walked `model_meta.json` for each entry, and fired ~5,500 HF API calls per release run — a full ~700-candidate pass at concurrency=8 burned through anonymous (500/5min) and free-tier (1,000/5min) HF rate limits and tripped 429s mid-run. The Python rewrite replaces the entire bulk path with `mteb.get_model_metas()` (the in-process ModelMeta registry the MTEB maintainers curate by hand). Each entry exposes `name`, `max_tokens`, and `loader_kwargs.model_prompts` directly — exactly the three load-bearing fields we need. Two file-format quirks made the Python helper non-optional: (1) `bge_models.py` uses literal `"query"` string keys in the prompts dict, but `e5_models.py` uses `PromptType.query.value` enum-attribute keys; both resolve to the runtime string `"query"` because `PromptType` is a `str` enum, and importing the actual registry resolves both forms uniformly. (2) Some families build the prompts dict via conditional code that pure-JS regex of the source files cannot follow. Schema bumped v1 → v2: drop `dim` / `prefixSource` / `modelType` / `baseModel` / `hasDenseLayer` / `hasNormalize` / `sizeBytes` / `runnableViaTransformersJs` (all display-only — runtime probes `dim` from the loaded ONNX, and verified during the rewrite that MTEB's curated `embed_dim=512` for `BAAI/bge-small-en-v1.5` is wrong; actual model dim is 384). The `seed-loader.ts` reads both v1 and v2 (committed anchor adapter on read). 348 models in the regenerated seed (up from the hand-curated 6-entry anchor); covers all canonical presets plus hand-aliased Xenova mirrors. `npm run build:seed` is preserved as a one-line wrapper around `python3 scripts/build-seed.py` so local invocation stays one short command. Release CI runs `actions/setup-python@v5` + `pip install 'mteb>=2.12,<3'` + `npm run build:seed` with `continue-on-error: true` — a future MTEB symbol rename never blocks a release because the committed anchor ships unchanged in that case. The v1→v2 entry adapter (`_adaptV1Entry`) is exported and unit-tested with both asymmetric (BGE) and symmetric (bge-m3) v1-shaped fixtures so the back-compat branch is genuinely covered, not aspirational — closes a finding from the post-implementation audit (the committed anchor is always v2 so the live `loadSeed()` path never exercises the adapter; only direct unit tests can keep it honest).
...
- **`docs/models.md` drift fixed** — three flagged-by-audit issues: (1) `english` preset prefix description wrong (said `query:`/`passage:` E5-style; actual seed has BGE's `Represent this sentence for searching relevant passages: ` / `""`). (2) Seed coverage wrong (said "~250"; actual is 348 since the Python rewrite). (3) `models check` description wrong (said it goes through "the same resolution chain the runtime uses"; actually it skips the chain and goes direct to live HF — verified via the CLI audit). Plus added the new `models add` / `models override` / `models fetch-seed` rows to the subcommand table.
- **Tests** — 4 new test files covering each layer with mocked fetch + in-memory DB (35 new test cases at original ship; +2 cases later for the v1→v2 seed adapter). Existing tests updated for the call-site changes. Plus the CLI / resolver / error pass: 5 new vitest cases for `models add` (set, defaults, refuses-seeded, refuses-existing-override, rejects-zero), 2 for the resolver short-circuit (complete-override skips HF; partial-override still hits HF), 5 for the `UserError` formatter, and snapshot regens for the new `models add` help text. Plus a regression-pin inline-snapshot test on the `models list` JSON output that locks the exact six-preset table (`preset` / `model` / `provider` / `maxTokens` / `symmetric`) — if a preset's underlying model id changes, MTEB updates an upstream `max_tokens`, or a new preset is added/removed, the snapshot diff forces an explicit "yes I meant this" in the PR (with a comment block in the test pointing at `docs/models.md` and `docs/cli.md` as the matching prose to update). Total 872/872 vitest passing.
...
- **Stripped `v1.7.5` from the live CLI `models refresh-cache --help` description** — the surface visible to users on `obsidian-brain models refresh-cache --help` no longer carries internal version refs, matching the new doc-prose rule. Help-snapshot test names also de-version-suffixed (`v1.7.5 fix:` → `regression:`). Plus a new Python unit-test suite for `scripts/build-seed.py` (`test/scripts/test_build_seed.py`, 18 stdlib-`unittest` cases covering the filter rules — open-weights / dense / modality / multi-vector / static-embedding — and the prompt-extraction edge cases — bge-style literal `"query"` keys, e5-style `PromptType.query.value` enum keys, `"passage"` fallback for older configs, symmetric-model null-prefix path, `max_tokens` int coercion + skip reasons, defensive non-string prompt-value rejection, and the alias-table covers-every-canonical-preset invariant). Runs in 0.2s with stdlib only — no `mteb` import needed because the helpers use duck-typed fake `ModelMeta` objects. Wired into `npm run preflight` and `.github/workflows/ci.yml` so a regression in the seed generator fails CI on every PR; `npm run test:python` runs it standalone.
...
- **`models list` surfaces every seed entry, not just the 6 presets** — added `--all` (include every entry in the bundled MTEB-derived seed; ~348 dense, text-only, open-weights models as of mteb 2.12.30) and `--filter <substr>` (case-insensitive substring match on model id) flags. Combinable: `models list --all --filter mongodb` returns both `MongoDB/mdbr-leaf-ir` (preset: `english-fast`) and `MongoDB/mdbr-leaf-mt` (preset: `null`). Output adds a `preset` field that's `null` for non-preset entries; TTY footer shows "(N models matching <q> — pass --all for every seed entry)" so the gap is obvious. Closes the v1.7.5-shipping discoverability gap where the seed we just shipped was invisible to users.
- **`models refresh-cache` description tightened** — was missing two real-world facts. Added: (1) for seeded models the cost is ~0 HF calls (the 348-entry seed repopulates the cache instantly on next boot); 1 HF call per non-seeded BYOM id. (2) the prefix-strategy hash auto-detects any prefix change and triggers a re-embed in bootstrap, so it's safe to run any time. Caveat: if you run it OFFLINE on a non-seeded BYOM id, fallback safe defaults get cached — fix by running again online.
- **New `docs/cli.md` page — first centralised CLI reference** covering every subcommand under `obsidian-brain` (`server`, `index`, `watch`, `search`, all `models` subcommands). Wired into `mkdocs.yml` nav as its own top-level "CLI" section. Includes the resolution-chain doc ("How model metadata is resolved") so users understand where each command sits in the override → cache → seed → HF → embedder-probe → fallback flow.
- **User-config layer — overrides + user-fetched seed, both survive `npm update`** (`src/embeddings/user-config.ts`, `src/embeddings/overrides.ts`). Two new files live in `~/.config/obsidian-brain/` (XDG-compliant; `%APPDATA%/obsidian-brain/` on Windows; `$OBSIDIAN_BRAIN_CONFIG_DIR` overrides everything). Because they're outside the npm package directory, both survive package updates intact. Resolver chain extended from 5 layers to 6: **override (Layer 0)** → cache → seed (user-fetched > bundled) → HF → embedder probe → fallback. Override changes auto-trigger a re-embed via the existing prefix-strategy hash in `bootstrap.ts`; `maxTokens` overrides take effect on the next reindex.
- **New `obsidian-brain models override <id>` subcommand** — set, remove, or list per-model metadata overrides. Three modes: `--max-tokens N --query-prefix S --document-prefix S` (set/patch — flags combinable), `--remove [--field name]` (clear all or one field), `--list` (dump every override on disk). Each override is a partial patch — omitted fields fall through to the next layer. Use case: MTEB's curated `embed_dim=512` for `BAAI/bge-small-en-v1.5` is wrong (actual is 384) — a user wanting to correct a similar bug locally now runs `models override <id> --max-tokens <correct>` and the change persists across `npm update`. Validation rejects non-positive `maxTokens` and non-string non-null prefixes per-field; bad fields drop, good fields keep.
- **New `obsidian-brain models fetch-seed` subcommand** — download the latest `data/seed-models.json` from the `main` branch on GitHub and write it to `~/.config/obsidian-brain/seed-models.json`. The seed-loader now checks the user-fetched path **before** the bundled npm-tarball copy, so users get upstream MTEB fixes without waiting for an npm release. `--check` validates the download without writing. `--url <url>` overrides the source for forks / self-hosted mirrors. Schema-version-aware — refuses to overwrite if the fetched file declares an unsupported `$schemaVersion` (forces a package upgrade for runtime-affecting schema changes). Atomic write (`tmp` + rename) so partial downloads can never leave a corrupt seed.
...
- **Tests** — 17 new vitest cases for the user-config layer: 14 in `test/embeddings/overrides.test.ts` (round-trip, partial merge, single-field remove, validation rejects bad shapes, accepts null-as-clear, unsupported $version ignored) and 5 in `test/embeddings/metadata-resolver.test.ts` (override.maxTokens replaces seed value, override.queryPrefix replaces only that field, null prefix as explicit-clear, no-override preserves original prefixSource, override applies on cache-hit not just first-resolve). Total 844/844 vitest passing.
- **Smoke-tested the new Python build path end-to-end** — local run with mteb 2.12.30 produces a 348-entry seed in ~5 seconds, zero HF API calls. All six canonical presets (Xenova/bge-small/base, Xenova/multilingual-e5-small/base, MongoDB/mdbr-leaf-ir, bge-m3) resolve with correct prefixes (BGE: `"Represent this sentence for searching relevant passages: "` query / empty document; E5: `"query: "` / `"passage: "`; bge-m3: null/null symmetric). `release.yml` runs the regen step in CI between `npm run build` and `npm publish` with `continue-on-error: true` — if a future MTEB release renames `get_model_metas` or restructures `loader_kwargs`, the committed anchor still ships, observable in the workflow log without blocking the release.
...
- **CLI help-text accuracy pass** — three lies fixed: (1) `obsidian-brain index --drop` description claimed it was "required when switching EMBEDDING_MODEL" — false since v1.4.0 (bootstrap auto-detects model/provider changes and wipes state on its own); now says "mostly an escape hatch". (2) `obsidian-brain search` only listed `semantic | fulltext` modes despite `ctx.search.hybrid()` being the production default since v1.4.0 — `--mode hybrid` (RRF-fused) is now the default and listed first; explicit unknown-mode rejection added. (3) `obsidian-brain models prefetch` declared a `--timeout <ms>` option that was `void`-ed in the action (option existed, did nothing, lied to users about what it did) — option removed; `prefetchModel` has its own retry+backoff loop. Verified by walking every subcommand's `--help` and cross-referencing source.
