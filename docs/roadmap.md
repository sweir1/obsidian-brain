...
- **Auto `ollama pull` on missing model (default).** Today users hitting `multilingual-ollama` for the first time have to run `ollama pull qwen3-embedding:0.6b` manually after seeing v1.7.19's actionable error. v1.7.21 makes this automatic: when `EMBEDDING_PROVIDER=ollama` and `/api/show` returns 404 for the configured model, kick off `/api/pull` (streaming JSON-line response → progress to stderr) and re-init once it completes. Default ON — choosing an Ollama preset is consent to download its model, and Ollama's own ergonomic (`ollama run` auto-pulls) sets the precedent. Opt-out via `OBSIDIAN_BRAIN_OLLAMA_AUTO_PULL=0` for users who want to manage pulls themselves.
