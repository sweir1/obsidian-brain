...
- Schema: `src/store/db.ts` — `nodes`, `edges`, `communities`, `sync`, `chunks`, `index_metadata`, plus `embedder_capability` and `failed_chunks` (both added in v1.7.0's schema v6 for the adaptive capacity + fault-tolerant embed loop), and the FTS5 virtual table `nodes_fts` (tokenizer `porter unicode61` since v1.4.0) and two sqlite-vec virtual tables: `nodes_vec` (mean-pooled note-level vector, kept for compat) and `chunks_vec` (per-chunk vectors, the main retrieval target since v1.4.0). The vec0 dim is reconciled against the embedder at runtime and `index_metadata` records the active embedding model/dim/provider so a mismatch on reboot triggers an auto-reindex (`src/pipeline/bootstrap.ts`). `embedder_capability` caches per-model `(advertised_max_tokens, discovered_max_tokens, method)` keyed on `(embedder_id, model_hash)` so the chunker picks a safe token budget without re-probing on every boot; `failed_chunks` records `(chunk_id, note_id, reason, error_message, failed_at)` for any chunk the fault-tolerant loop skipped (surfaced via the `index_status` tool). Since v1.2.2 `deleteNode` also cascades to the `communities` table via `pruneNodeFromCommunities` (`src/store/communities.ts`); since v1.4.0 that prune also regenerates each affected cluster's `summary` string via `buildSummary` so `nodeIds` and `summary` stay consistent.
